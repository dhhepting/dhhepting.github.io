    ...
    if kind in people:
        ...
        if kind == "author":
            for pp in range(len(ppl)):
                ...
                #print('\"' + fnamestr + mnamestr + plnamestr + lnamestr + '\",\"' + globprojstr + '\"')
                lnk = plnamestr + lnamestr
                fnk = fnamestr + mnamestr
                pk = lnk.strip() + ", " + fnk.strip()
                if (pk not in ppldict):
                    ppldict[pk] = []
                for prj in globprojstr.split(","):
                    prjs = prj.strip()
                    #print (pk,prjs,globprojstr)
                    if prjs not in ppldict[pk]:
                        ppldict[pk].append(prjs)
            #mdf.write(perstr + "\n")
...
for p in ppldict:
    prstr = ""
    #print(p, ppldict[p], len(ppldict[p]))
    if len(ppldict[p]) > 0:
        for i in range(len(ppldict[p]) - 1):
            prstr += (ppldict[p][i] + ",")
        prstr += ppldict[p][len(ppldict[p])-1]
    print('\"' + p + '\",\"' + prstr + '\"')
