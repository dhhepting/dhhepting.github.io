/* eslint-disable require-jsdoc */
// based on https://www.interactivecomputergraphics.com/8E/Code/02/gasket2.js

'use strict';

// global declarations
let gl;
const positions = [];
const colours = [];

const xforms = [
  mat3(),
  mat3(),
  mat3()
];

const baseColours = [
  vec4(1,0,0,1),
  vec4(0,1,0,1),
  vec4(0,0,1,1)
];
const centers = [
  vec3(-0.5, -0.2887, 1.0),
  vec3(0.5, -0.2887, 1.0),
  vec3(0.0, 0.5773, 1.0)
];

// First, initialize the matrices of our gasket transformations
  xforms[0] = mult(xforms[0],translate(centers[0][0],centers[0][1]));
  xforms[0] = mult(xforms[0],scale(0.5,0.5));
  xforms[0] = mult(xforms[0],rotate(45);
  xforms[0] = mult(xforms[0],translate(-centers[0][0],-centers[0][1]));
  xforms[1] = mult(xforms[1],translate(centers[1][0],centers[1][1]));
  xforms[1] = mult(xforms[1],scale(0.5,0.5));
  xforms[1] = mult(xforms[1],translate(-centers[1][0],-centers[1][1]));
const numTimesToSubdivide = 3;

window.onload = function init() {
  const canvas = document.getElementById('gl-canvas');
  gl = canvas.getContext('webgl2');
  if (!gl) {
    alert('WebGL 2.0 is not available');
  }

  //
  //  Create our data for the Sierpinski Gasket
  //

  recursivePts(mat3(),0,10);

  //
  //  Configure WebGL
  //
  gl.viewport(0, 0, canvas.width, canvas.height);
  gl.clearColor(0.5, 0.5, 0.5, 1.0);

  //  Load shaders and initialize attribute buffers

  const program = initShaders(gl, 'vertex-shader', 'fragment-shader');
  gl.useProgram(program);

  // Load the data into the GPU

  const bufferId = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, bufferId);
  gl.bufferData(gl.ARRAY_BUFFER, flatten(positions), gl.STATIC_DRAW);

  // Associate out shader variables with our data buffer

  const positionLoc = gl.getAttribLocation(program, 'aPosition');
  gl.vertexAttribPointer(positionLoc, 2, gl.FLOAT, false, 0, 0);
  gl.enableVertexAttribArray(positionLoc);

  const cBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, cBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, flatten(colours), gl.STATIC_DRAW);

  const colourLoc = gl.getAttribLocation(program, 'aColor');
  gl.vertexAttribPointer(colourLoc, 4, gl.FLOAT, false, 0, 0);
  gl.enableVertexAttribArray(colourLoc);

  render();
};

function recursivePts(currTmat,fxf,count) {
  if (count > 0) {
    // compose the transformations
    // by pre-multiplying the new transformations with the current
    recursivePts(mult(xforms[0],currTmat), 0, count - 1);
    recursivePts(mult(xforms[1],currTmat), 1, count - 1);
    recursivePts(mult(xforms[2],currTmat), 2, count - 1);
  } else {
    let hp = mult(currTmat,centers[0]);
    let p2d = vec2(hp[0],hp[1]);
    positions.push(p2d);
    colours.push(baseColours[fxf]);
  }
}
function scale_by_half() {
  return (mat3( 
    0.5, 0.0, 0.0,
    0.0, 0.5, 0.0,
    0.0, 0.0, 1.0
));
}
function render() {
  gl.clear(gl.COLOR_BUFFER_BIT);
  gl.drawArrays(gl.POINTS, 0, positions.length);
}
